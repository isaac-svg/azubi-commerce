...
import { Button, QuantityInput, useCart } from "@/components";
      ...
      <div className="my-20">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {product.others.map((item) => {
            // Find the actual product to get the category
            const actualProduct = recommendedProducts.find(p => p.slug === item.slug);
            return (
              <div key={item.slug} className="text-center">
                ...
                <Link
                  href={`/${actualProduct?.category || 'products'}/${item.slug}`}
                >
